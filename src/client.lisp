    ...
    (macrolet ((ignore-some-errors (&body body)
                 `(handler-case
                    ...
                    (socket-not-connected-error ()
                      (log:debug "enotconn ~s ~s:~s ~s" ,(format nil "~s" body)
                          (client-host client) (client-port client) fd)
                      nil)
                    (isys:epipe ()
                      (log:debug "epipe in disconnect ~s:~s" (client-host client) (client-port client))
                      nil)
                    (isys:enotconn ()
                      (log:debug "enotconn in shutdown/close? ~s:~s" (client-host client) (client-port client))
                      nil))))
      ...
      (flet ((enable ()
        ...
        (handler-case
          ...
          (isys:epipe ()
            ...
            (log:debug "epipe ~s:~s" (client-host client) (client-port client))
            (client-enqueue-read client (list client :dropped))
            (client-disconnect client :close t))
          (socket-connection-timeout-error ()
            (log:debug "connection timeout ~s:~s" (client-host client) (client-port client))
            (client-enqueue-read client (list client :dropped))
            (client-disconnect client :close t))
          (socket-host-unreachable-error ()
            (log:debug "host unreachable ~s:~s" (client-host client) (client-port client))
            (client-enqueue-read client (list client :dropped))
            (client-disconnect client :close t))
          (socket-connection-reset-error ()
            (log:debug "connection reset ~s:~s" (client-host client) (client-port client))
            (client-enqueue-read client (list client :dropped))
            (client-disconnect client :close t)))))))
  ...
  (unless (client-write-closed client)
    (cond
      ...
      ((> (mailbox-count (client-write-queue client))
          *max-write-backlog*)
       (log:debug "client ~s:~s write backlog = ~s, killing connection"
           (client-host client) (client-port client) (mailbox-count (client-write-queue client)))
